...
from src.processing.generate import format_instance, get_sentences, generate_prefix, generate_instructions, generate_demonstrations, generate_prediction
...
def main():
    ...
    for example in tqdm(valid): 
        ...
        for sentence, tagged_sentence in tqdm(zip(get_sentences(abstract), get_sentences(tagged_abstract), strict=True)):
            ...
            s_time = time()
            predicted_response = generate_prediction(model, tokenizer, prefix, input, kind)
            e_time = time()
    ...
    union_precision = round(n_tp_union / (n_tp_union + n_fp_union) if (n_tp_union + n_fp_union) > 0 else 0, 4)
    union_recall = round(n_tp_union / (n_tp_union + n_fn_union) if (n_tp_union + n_fn_union) > 0 else 0, 4)
    ...
    with open(os.path.join(res_dir, fname), "w", encoding="utf-8") as f:
        for input, predicted_response, gold_tag, pred_tag in zip(all_inputs, predicted_responses, gold_tags, predicted_tags):
            f.write(f"Prompt:\n{prefix + input}\n")
            f.write(f"True Tag:\n{gold_tag}\n")
